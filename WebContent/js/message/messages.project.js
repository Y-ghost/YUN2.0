...
rainet.message.controller.project = {
	...
	setValidateForPrjoect : function($form, operate){
		...
		.on('change', '.provinceItem', function(){
			...
			rainet.message.service.project.validName(data, function(data){
				if (data) {
					// 存在，更新错误信息的提示
  ...
  table : {
	...
	detail : function(self){
		...
		rainet.message.service.project.get(projectId, function(data){
			_this.setProjectInfo(data, true);
		});
	},
	// 更新单个项目信息
	edit : function(self, $datatable){
		...
		var _this = this;
		rainet.message.service.project.get(projectId, function(data){
			_this.setProjectInfo(data, false, $datatable);
		});
	},
	// 删除某个项目
